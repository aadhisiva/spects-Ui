...
export default function SecondaryScreeningReports() {
  ...
  const getDataFromApi = async () => {
    const { DataType, DistrictCode, TalukCode, FromDate, ToDate } = searchObject
  ...
  const handleDownloadReports = async (values: any) => {
    const { DataType, DistrictCode, TalukCode, FromDate, ToDate } = values
    setLoading(true)
    await PostRequestWithdownloadFile(
      'downloadStateOrDistrictReportsOfV2',
      {
        DataType: DataType || null,
        DistrictCode: DistrictCode || null,
        TalukCode: TalukCode || null,
